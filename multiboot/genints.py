...
def main():
    def generate_entry(intno, witherr, common):
        err = '\tpushq ${}\n\tjmp {}'.format(intno, common)
        noerr = '\tpushq $0\n\tpushq ${}\n\tjmp {}'.format(intno, common)

        return [noerr, err][bool(witherr)]
    ...
    def generate_common(regs, handler):
        ...
        return prologue + save_regs + call + restore_regs + epilogue

    # For these interrupts CPU puts 'error code' on stack in
    # addition to everything else, so they require special care
